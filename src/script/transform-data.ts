...
for (let i = 0; i < 30; i++) {
    creatures.push(
        new Creature(
            ...
            previousCreatures[i]?.tags?.map(CreatureTag.fromJSON) ?? [
                CreatureTag.GEN1,
            ],
            previousCreatures[i]?.types ?? [],
            new Text(
                previousCreatures[i]?.origin?.value ?? "",
                previousCreatures[i]?.origin?.words?.map(
                    (x: any) => new Word(x.romaji, x.kana, x.kanji)
                ) ?? []
            ),
...
fs.writeFileSync(
    "./script-out/generate-empty-out.json",
